...
public interface TaskRepository extends JpaRepository<Task, UUID> {
        ...
        Page<Task> findAllByProjectAndArchivedFalse(Project project, Pageable pageable);

        Page<Task> findAllBySprintAndArchivedFalse(Sprint sprint, Pageable pageable);

        Page<Task> findAllByProjectAndAssigneeAndArchivedFalse(Project project, User assignee, Pageable pageable);

        Page<Task> findAllByProjectAndStatusAndArchivedFalse(Project project, TaskStatus status, Pageable pageable);

        @Query("SELECT COALESCE(MAX(t.sequenceNumber), 0) FROM Task t WHERE t.project = :project")
        Integer findMaxSequenceNumberByProject(@Param("project") Project project);
